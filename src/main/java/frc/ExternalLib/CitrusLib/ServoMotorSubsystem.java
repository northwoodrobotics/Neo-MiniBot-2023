...
import frc.ExternalLib.PoofLib.drivers.TalonFXFactory;
import frc.ExternalLib.PoofLib.drivers.TalonUtil;
import frc.ExternalLib.PoofLib.motion.MotionProfileConstraints;
import frc.robot.Constants;
import frc.ExternalLib.PoofLib.motion.MotionProfileGoal;
import frc.ExternalLib.PoofLib.motion.MotionState;
import frc.ExternalLib.PoofLib.motion.SetpointGenerator;
import frc.ExternalLib.PoofLib.motion.SetpointGenerator.Setpoint;
import frc.ExternalLib.PoofLib.util.Util;
import frc.ExternalLib.CitrusLib.CitrusConstants;
import frc.ExternalLib.CitrusLib.loops.ILooper;
import frc.ExternalLib.CitrusLib.loops.Loop;
...
public abstract class ServoMotorSubsystem extends Subsystem {
    ...
    protected ServoMotorSubsystem(final ServoMotorSubsystemConstants constants) {
        ...
        TalonUtil.checkError(
                mMaster.config_IntegralZone(kMotionProfileSlot, mConstants.kIZone, CitrusConstants.kLongCANTimeoutMs),
                mConstants.kName + ": Could not set i zone: ");

        TalonUtil.checkError(
                mMaster.configAllowableClosedloopError(kMotionProfileSlot, mConstants.kDeadband,
                        CitrusConstants.kLongCANTimeoutMs),
                mConstants.kName + ": Could not set deadband: ");

        TalonUtil.checkError(
                mMaster.config_kP(kPositionPIDSlot, mConstants.kPositionKp, CitrusConstants.kLongCANTimeoutMs),
                mConstants.kName + ": could not set kP: ");

        TalonUtil.checkError(
                mMaster.config_kI(kPositionPIDSlot, mConstants.kPositionKi, CitrusConstants.kLongCANTimeoutMs),
                mConstants.kName + ": could not set kI: ");

        TalonUtil.checkError(
                mMaster.config_kD(kPositionPIDSlot, mConstants.kPositionKd, CitrusConstants.kLongCANTimeoutMs),
                mConstants.kName + ": could not set kD: ");

        TalonUtil.checkError(
                mMaster.config_kF(kPositionPIDSlot, mConstants.kPositionKf, CitrusConstants.kLongCANTimeoutMs),
                mConstants.kName + ": Could not set kF: ");
        ...
        TalonUtil.checkError(
                mMaster.config_IntegralZone(kPositionPIDSlot, mConstants.kPositionIZone,
                        CitrusConstants.kLongCANTimeoutMs),
                mConstants.kName + ": Could not set i zone: ");
        ...
        TalonUtil.checkError(
                mMaster.configMotionAcceleration(mConstants.kAcceleration, CitrusConstants.kLongCANTimeoutMs),
        ...
        // TalonUtil.checkError(
        // mMaster.configPeakCurrentDuration(mConstants.kPeakCurrentDuration,
    ...
    public synchronized void setGoalMotionProfiling(MotionProfileGoal goal, double feedforward_v) {
        ...
        Setpoint setpoint = mSetpointGenerator.getSetpoint(mMotionProfileConstraints, goal, cur_state,
                mPeriodicIO.timestamp + CitrusConstants.kLooperDt);
    ...
    public synchronized void zeroSensors() {
        mMaster.setSelectedSensorPosition(0, 0, CitrusConstants.kCANTimeoutMs);
